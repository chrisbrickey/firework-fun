...
class Firework {
  ...
  buildPelletArray(startX, startY) {
    ...
    for(let i = 0; i < 200; i++) {
      ...
      let colorValues = this.colors[Math.floor(i % numColors)]; //result = array of three integers
      let alpha = 1.0;
      let pellet = new Pellet(x, y, this.pelletRadius, `rgba(${colorValues[0]}, ${colorValues[1]}, ${colorValues[2]}, ${alpha})`);
